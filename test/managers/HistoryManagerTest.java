...
class HistoryManagerTest {
    ...
    @BeforeEach
    void createManagerAndTasks_shouldCreateManagerAndTasks() {
    ...
    @Test
    void getHistory_shouldReturnHistoryContainsViewedTask_whenViewedOneTask() {
        manager.getTask(task.getId());
        assertTrue(manager.getHistory().contains(task), "История просмотров не содержит просмотренную задачу");
    }

    @Test
    void getHistory_shouldReturnHistorySize3_whenViewThreeDifferentTasks() {
        manager.getTask(task.getId());
    ...
    @Test
    void getHistory_shouldReturnHistorySize2_whenViewThreeTasksTwoOfWhichTheSame() {
        manager.getTask(task.getId());
        manager.getSubtask(subtask1.getId());
        manager.getSubtask(subtask2.getId());
    ...
    @Test
    void getHistory_shouldReturnHistorySize2_whenViewThreeDifferentTasksAndRemovedLast() {
        manager.getTask(task.getId());
        manager.getSubtask(subtask1.getId());
        manager.getSubtask(subtask2.getId());
    ...
    @Test
    void getHistory_shouldReturnHistorySize1AndContainsViewedEpicAndDoesNotContainsEpicSubtasks_whenViewedEpic() {
    ...
    @Test
    void getHistory_shouldReturnHistorySize1_whenViewOneEpicTwice() {
        manager.getEpic(epic.getId());
        manager.getEpic(epic.getId());
        assertEquals(1, manager.getHistory().size(),
    ...
    @Test
    void getHistory_shouldReturnEmptyListHistory_whenNoViews() {
